...
#define DEST_IP (dest_ip_1 | (dest_ip_2 << 8) | \
    (dest_ip_3 << 16) | (dest_ip_4 << 24))
...
bool handle_packet(uint8_t *buf, nat_table *nat_table,
                   struct int_info *int_info, uint16_t *port_cntr) {
    ...
    if(ntohs(*trans_hdr_map.port_dst) == listen_port) {
        ...
        if(!nat_entry) {
            ...
            if(nat_entry_new.port_alloc == 0) {
                ...
                if(!nat_table_remove_if(nat_table, nat_entry_new.alloc_time,
                                        &cond_time)) {
        ...
        *net_hdr_map.checksum = recompute_checksum_32(*net_hdr_map.checksum,
        ...
        if(*net_hdr_map.next_proto == 6) {
            ...
            *trans_hdr_map.checksum = recompute_checksum_32(
                *trans_hdr_map.checksum, *net_hdr_map.addr_dst, DEST_IP);
        }
    ...
    /* Server to client packet */
    } else if(*trans_hdr_map.port_src == htons(dest_port) &&
              *net_hdr_map.addr_src == DEST_IP) {
#ifndef ENABLE_LOG_DEBUG
        LOG_INFO("Packet\n");
#endif
        ...
        *trans_hdr_map.checksum = recompute_checksum_16(
            *trans_hdr_map.checksum, *trans_hdr_map.port_src, htons(listen_port));
        *trans_hdr_map.checksum = recompute_checksum_16(
